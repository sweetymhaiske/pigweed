...
with open(compile_commands_path) as compile_commands_json:
    ...
    def get_compile_flags(src_file):
        ...
        print(compile_flags)
        length = len(compile_flags) 
        for i in range(length):
            str = compile_flags[i]
#            print("reverse %s\n",str[-8:])
            print("S T R I N G %s",str)
            if str[:3] == "-I.":
                compile_flags[i] = compile_flags[i].replace("-I..", "-I/home/sweety/Espressif/connectedhomeip/examples/pigweed-app/esp32")
            elif str[:2] == "-I" and str[:3] == "-I/" and str[:32] == "-I/home/sweety/Espressif/esp-idf":
                compile_flags[i] = compile_flags[i].replace("-I", "-isystem")
            elif str[:9] == "-Iesp-idf":
                compile_flags[i] = compile_flags[i].replace("-Iesp-idf", "-I/home/sweety/Espressif/connectedhomeip/examples/pigweed-app/esp32/third_party/connectedhomeip/examples/pigweed-app/esp32/build")
            if str[-8:] == "freertos":
                compile_flags[i] = "\"" + compile_flags[i] + "/\""
            else:
                compile_flags[i] = "\"" + compile_flags[i] + "\""
            print(compile_flags[i])
